...
class Grapher:
    ...
    def __init__(self, filename, data_fd):
        ...
        interim_path = os.path.join(self.data_fd, "labels", filename + '.csv')
        image_path = os.path.join(self.data_fd, "img", filename + '.jpg')
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            self.df = pd.DataFrame(lines, columns=[0])
        except FileNotFoundError:
            print(f"LỖI: Không tìm thấy file: {file_path}")
            print("Vui lòng kiểm tra lại giá trị của file_id và DATA_FOLDER.")
            raise
